...
export async function importTradesView(root) {
  ...
  const step3 = () => {
    ...
    const rows = p.rows.map((r) => h("tr", { class: r.status !== "OK" ? "is-current" : "" },
      ...
      h("td", { class: "mono small" }, r.original_date), h("td", { class: "mono small" }, r.original_time),
    ...
    mount(body,
      h("div", { class: "panel" },
        ...
        table(["Position", "Instrument", "Original date", "Original time", "Normalized entry", "TZ source", "Entry", "Direction", "Trade ID", "Status"], rows)),
